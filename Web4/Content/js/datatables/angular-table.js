...
(function() {
    var ColumnConfiguration,
        PageSequence, PaginatedSetup,
        ScopeConfigWrapper, Setup,
        StandardSetup, Table, TableConfiguration,
        configurationVariableNames, paginationTemplate,
  ...
  Setup = (function() {
    ...
    Setup.prototype.setupTr = function(element, repeatString) {
      ...
      tr = tbody.find("tr");
     // tr.attr("ng-if", "Item_Count");
      tr.attr("ng-repeat", repeatString);
   //   tr.attr("ng-click", "setNotes(item)");
   //   tr.attr("ng-class", "{selected: item.Customer_ID == selectedId }");
  ...
  StandardSetup = (function(_super) {
    ...
    function StandardSetup(configurationVariableNames, list) {
        this.list = list;
        //Added the ability to 'filter the results by input box entry (VT: 03/27/2015)';
  ...
  PaginatedSetup = (function(_super) {
    ...
    function PaginatedSetup(configurationVariableNames) {
      this.configurationVariableNames = configurationVariableNames;
      this.repeatString = "item in sortedAndPaginatedList | filter:search";
  ...
  angular.module("angular-table").directive("atSearchFilter", [
      "$filter", function ($filter) {
          return {
              restrict: "AC",
              scope: true,
  ...
  angular.module("angular-table").directive("atImplicit", [
    function() {
      return {
        restrict: "AC",
        compile: function(element, attributes, transclude) {
          var attribute, accordion, invono;
          ...
          if (!attribute) {
            ...
            //Check for accordion.
          if (element.attr("at-accordion")) {
              //Add the accordion.
             
              element.append('&nbsp;<a class="collapsed" aria-expanded="false" href="#collapse-{{item.$id}}" data-parent="#accordion-{{item.$id}}" data-toggle="collapse"><span class="glyphicon glyphicon-plus">&nbsp;</span></a>');
              element.append('<div id="collapse-{{item.$id}}" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;"><div class="panel-body"><table><thead><tr><th>Qty</th><th>Item ID</th><th>Name</th><th>Price</th></tr></thead><tbody><tr><td>{{item.Qty}}</td><td>{{item.Item_ID}}</td><td>{{item.Web_Name}}</td><td>{{item.Price}}</td></tr></tbody></table><table><thead><tr><th>Eye</th><th>Color</th><th>Base</th><th>Diam.</th><th>Cyl</th><th>Axis</th><th>Other</th></tr></thead><tbody><tr><td>{{item.Eye}}</td><td>{{item.Color}}</td><td>{{item.BaseCurve}}</td><td>{{item.Diameter}}</td><td>{{item.Cylinder}}</td><td>{{item.Axis}}</td><td>{{item.Other}}</td></tr></tbody></table></div>');              
              element.append('</div>');
          ...
          element.prepend("{{item." + attribute + "}}");
          return element;
        }
      };
    }
  ...
  angular.module("angular-table").directive("atRefundme", [
   function () {
       return {
           restrict: "AC",
           compile: function (element, attribute, transclude) {
               var attribute;
               attribute = element.attr("at-refundme");              
               var refundAmt = element.attr("at-refundamount");
               return element.append("<a ng-click='refundme(item." + attribute + ")' class='btn btn-secondary btn-sm btn-icon icon-left'>Refund</a><input ng-model='refundamt' ng-change='setRefundAmt()' autofocus />");
 ...
 // NEW DIRECTIVE FOR INTEGRATING THE PRINT BUTTON / TIE INTO THE PDF PRINTING

  angular.module("angular-table").directive("atPrint", [
   function () {
       return {
           restrict: "AC",
           compile: function (element, attribute, transclude) {
               var attribute;
               attribute = element.attr("at-print");             
               return element.append("<a ng-click='print(item."+attribute+")' class='btn btn-secondary btn-sm btn-icon icon-left'>Print</a>");              
           }
       }
   }
  ]);

  angular.module("angular-table").directive("atAdditem", [
function () {
    return {
        restrict: "AC",
        compile: function (element, attribute, transclude) {
            var attribute, title;
            attribute = element.attr("at-attribute");
            title = element.attr("at-title");
            return element.append("<a ng-click='addItem(item." + attribute + ")' class='btn btn-secondary btn-sm btn-icon icon-left'>"+title+"</a>");
        }
    }
}
  ]);
  ...
  angular.module("angular-table").directive("atInputvalue", [
      function () {
          return {
              restrict: "AC",              
              compile: function (element, attribute, transclude) {
                  var attribute;
                  var modal;
                  attribute = element.attr("at-inputvalue");
                  if (!attribute) {
                      throw "at-inputvalue specified without at-attribute" + element.html();
                  }
                  return element.html("<span class='col-sm-4 form-control' ng-hide='item.editing' ng-dblclick='editItem(item)'>{{item."+attribute+"}}</span><input ng-show='item.editing' ng-model='item."+attribute+"' ng-blur='doneEditing(item)' autofocus />");
    ...
    /*      VT: 04/14/2015 :  
     *      Directive to capture the Prescription information and display a popup for 
     *      our csr's to make the call to client's doctor to verify prescription.
     *      Or, to print / access the jpg, or pdf file the user uploaded to the site.     * 
     *      atModal = Doctor, 
     *      atModal = Upload
     * ===============================================================================*/
  angular.module("angular-table").directive("atModal", [function () {
      return {
          restrict: "AC",
          compile: function (element, attribute, transclude) {
              var attribute;
              attribute = element.attr("at-Modal"); //value is prescription, or invoicedetails.
   
              if (attribute == "Prescription") {
                  return element.append('<div ng-if="item.IsPrescription"><a class="label label-success" ng-click="openPrescription(item)"><i class="glyphicon glyphicon-file">&nbsp;</i>Prescription Verify</a></div>');
              }
              if (attribute == "Refund") {
                  return element.append('<div ng-if="item.IsInvoice"><a class="label label-success" ng-click="openPrescription(item)"><i class="glyphicon glyphicon-file">&nbsp;</i>Refund Verify</a></div>');
              }
          }
      }
    ...
    /*
  angular.module("angular-table").directive("atAccordion", [function () {
      return {
          restrict: "AC",
          compile: function (element, attribute, transclude) {
              var attribute;
              attribute = element.attr("at-Accordion"); //value is prescription, or invoicedetails.
              if (attribute != "") {
                  element.append('<div><h4 class="bg-info"><a class="collapsed" aria-expanded="false" href="#collapse-{{item.$id}}" data-parent="#accordion-{{item.$id}}" data-toggle="collapse">View Details</a></h4>');
                  element.append('<div id="collapse-{{item.$id}}" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;"><div class="panel-body"><table><thead><tr><th>Qty</th><th>Item ID</th><th>Price</th><th>Name</th><th>Amount</th></tr></thead><tbody><tr><td>{{item.Qty}}</td><td>{{item.Item_ID}}</td><td>{{item.Price}}</td><td>{{item.Web_Name}}</td></tr></tbody></table></div></div>');
                  return element;
              }
          }
      }
